...
describe("allBooks", () => {
  ...
  test("filters books by genre", async () => {
    ...
    for (const book of result.data.allBooks) {
      assert.ok(
        book.genres.some((g) => g.name === "refactoring"),
...
describe("me", () => {
  test("returns null when not authenticated", async () => {
    const response = await server.executeOperation({
      query: "{ me { username favoriteGenre { name } } }",
    })
  ...
  test("returns current user when authenticated", async () => {
    ...
    const response = await server.executeOperation(
      {
        query: "{ me { username favoriteGenre { name } id } }",
      },
      {
    ...
    assert.strictEqual(result.data.me.favoriteGenre.name, "classic")

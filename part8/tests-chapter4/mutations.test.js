...
describe("createUser", () => {
  ...
  test("succeeds with valid data", async () => {
    ...
    assert.strictEqual(result.data.createUser.favoriteGenre.name, "classic")
...
describe("login", () => {
  ...
  test("returns token with correct credentials", async () => {
    ...
    const response = await server.executeOperation({
      query: LOGIN,
      variables: { username: "loginuser", password: "admin" },
    })
...
describe("addBook", () => {
  ...
  test("succeeds when authenticated", async () => {
    ...
    assert.deepStrictEqual(
      result.data.addBook.genres.map((g) => g.name),
      ["testing"],
    )
